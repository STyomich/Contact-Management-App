...
public class ContactsService(IUnitOfWork unitOfWork) : IContactsService
{
    ...
    public async Task ProcessCsvFileAsync(IFormFile csvFile, CancellationToken cancellationToken)
    {
        var contacts = Utils.CsvHelper.ReadCsv<AddContactRequest>(csvFile);

        foreach (var addContactRequest in contacts)
        {
            if (await _unitOfWork.ContactsRepository.IsExistsByPhoneAsync(addContactRequest.Phone, cancellationToken))
                continue; // Skip contacts with duplicate phone numbers

            var newContact = new Contact
            (
                id: Guid.NewGuid(),
                name: Name.Create(addContactRequest.Name).Value,
                dateOfBirth: DateOfBirth.Create(addContactRequest.DateOfBirth).Value,
                married: addContactRequest.Married,
                phone: Phone.Create(addContactRequest.Phone).Value,
                salary: Salary.Create(addContactRequest.Salary).Value
            );

            await _unitOfWork.ContactsRepository.AddAsync(newContact, cancellationToken);
        }
        await _unitOfWork.SaveChangesAsync(cancellationToken);
    }
